<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8"/>
<meta name="viewport" content="width=device-width, initial-scale=1.0"/>
<title>EasyFramework</title>
<%@include file="global.jsp"%>
<!-- jquery -->
<script type="text/javascript" src="${basePath }/js/plugins/jquery/jquery-1.10.1.min.js"></script>
<script type="text/javascript" src="${basePath }/js/plugins/jui/jquery-ui.min.js"></script>
<!-- 引入bootstrap样式 -->
<link rel="stylesheet" type="text/css" href="${basePath }/css/bootstrap/css/bootstrap.min.css" media="screen"/>
<link rel="stylesheet" type="text/css" href="${basePath }/css/bootstrap/css/bootstrap-responsive.min.css" media="screen"/>
<script type="text/javascript" src="${basePath }/css/bootstrap/js/bootstrap.min.js"/></script>

<!-- 引入easyui样式 -->
<script type="text/javascript" src="${basePath }/js/plugins/jeasyui/jquery.easyui.min.js"></script>
<script type="text/javascript" src="${basePath }/js/module/ext/easyui-ext.js"></script>
<link rel="stylesheet" type="text/css" href="${basePath }/css/themes/bootstrap/easyui.css"/>  
<link rel="stylesheet" type="text/css" href="${basePath }/css/themes/icon.css"/>  

<!-- 引入EasyUI Portal插件 -->
<link rel="stylesheet" href="${basePath }/js/plugins/jportal/portal.css" type="text/css">
<script type="text/javascript" src="${basePath }/js/plugins/jportal/jquery.portal.js" charset="utf-8"></script>

<!-- 下一个jquery模板 -->
<script type="text/javascript" src="${basePath }/js/plugins/jsviews/jsviews.min.js"></script>

<!-- 引入EasyFramework-->
<link rel="stylesheet" type="text/css" href="${basePath }/css/easyframework.css" />
<script type="text/javascript" src="${basePath }/js/easyframework-utils.js" ></script>

</head>
